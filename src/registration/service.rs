use prism_client::{
    binary::ToBinary as _, PendingTransaction, PrismApi, Signature, SignatureBundle, SigningKey,
    VerifyingKey,
};
use std::sync::Arc;
...
mod tests {
    ...
    async fn test_create_account() -> Result<()> {
        ...
        let service = RegistrationService::new(Arc::new(mock_client), service_signing_key.clone());
        ...
        let challenge_signature = user_identity_signing_key.sign(registration_challenge);
